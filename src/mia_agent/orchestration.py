...
from mia_agent.events import AgentErrorEvent, AgentEvent, AssistantChunkEvent
        ...
        *,
        ...
        try:
            ...
            async for event in runtime.harness.prompt(prompt_text):
                yield ModeRuntime._envelope(identity, event)
        except asyncio.CancelledError:
            yield ModeRuntime._error_envelope(identity, task_id, "prompt cancelled", cancelled=True)
        except Exception as exc:
            yield ModeRuntime._error_envelope(identity, task_id, str(exc))
        ...
        *,
        ...
        try:
            async for event in specialist.harness.prompt(prompt_text):
                ...
                yield ModeRuntime._envelope(specialist_identity, event)
                if isinstance(event, AgentErrorEvent):
                    return
        except asyncio.CancelledError:
            yield ModeRuntime._error_envelope(
                specialist_identity, "specialist", "prompt cancelled", cancelled=True
            )
            return
        except Exception as exc:
            yield ModeRuntime._error_envelope(specialist_identity, "specialist", str(exc))
            return
        ...
        try:
            async for event in coordinator.harness.prompt(handoff):
                yield ModeRuntime._envelope(coordinator_identity, event)
        except asyncio.CancelledError:
            yield ModeRuntime._error_envelope(
                coordinator_identity, "coordinator", "prompt cancelled", cancelled=True
            )
        except Exception as exc:
            yield ModeRuntime._error_envelope(coordinator_identity, "coordinator", str(exc))
